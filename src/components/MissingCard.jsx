...
const MissingCard = ({ data }) => {
  return (
    <>
      {data?.map((data, index) => {
        return (
          <Link to={`/single-missing-report/${data?._id}`} key={index} className="relative col-span-1  flex w-80 ml-1 flex-col rounded-xl bg-white bg-clip-border text-gray-700 shadow-md max-w-2xl mx-4 sm:max-w-sm md:max-w-sm lg:max-w-sm xl:max-w-sm sm:mx-auto md:mx-auto lg:mx-auto xl:mx-auto mt-0">
